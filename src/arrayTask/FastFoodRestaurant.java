...
//TASK 1
              int restaurantCount=restaurants.length-1;
...
//TASK 3
        ...
        int mcdonalsCount = 0;
for (String eachRestaurant:restaurants){
    if(eachRestaurant.contains("McDonald's")){
        mcdonalsCount++;
    }
}
...
for (String eachRest:restaurants){
...
//TASK 5
        String mcDonaldsTx="";
        int countMcDonaldsTx=0;
        for (String eachRst:restaurants){
            if(eachRst.contains(" McDonald's") && eachRst.contains("TX")){
                mcDonaldsTx+=" "+eachRst;
                countMcDonaldsTx++;
            }
        }
String[] mcDonaldsArray=mcDonaldsTx.split(" ");

        for (int i = 0; i <mcDonaldsArray.length ; i++) {
            System.out.println(i+1+". "+mcDonaldsArray);

        }
//TASK 6
        String[] firstRestaurantArray=restaurants[0].split(",");
        System.out.println(Arrays.toString(firstRestaurantArray));
   // "3022 5th Ave S,Fort Dodge,Jimmy Johns,50501,IA,http://www.jimmyjohns.com"
        //  "4461 Market Commons Dr,Fairfax,Pei Wei,22033,VA,http://www.peiwei.com",
       //         "14493 Gideon Dr,Woodbridge,Wendy's,22192,VA,http://www.wendys.com",
          //      "2815 Fern Valley Rd,Louisville,Arby's,40213,KY,http://www.arbys.com",
        for (String eachRest:restaurants){
           String[] eachrestArr=eachRest.split(",");
            System.out.print(eachrestArr[2]+" -> ");
            System.out.println(eachrestArr[4]);
        }
